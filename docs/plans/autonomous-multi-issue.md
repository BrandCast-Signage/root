...
| REQ-003 | Readiness grader runs before stream creation in autonomous mode and hard-stops on any issue scoring `not-ready`, surfacing per-issue clarification questions | P0 | `agents/issue-readiness-grader.md`, `skills/root/SKILL.md` |
...
| 2 | `mcp/mcp-root-board/src/__tests__/notify.test.ts` | create | All cases | Mock `fetch`. Test: feature-off when env var absent, feature-off when config section absent, payload shape per event type, swallows fetch errors, includes `mention` prefix when configured, omits when not. | REQ-005, REQ-008 | A | [ ] |
| 3 | `mcp/mcp-root-board/src/sharedContext.ts` | create | `getSharedContext(rootDir, epicIssue)`, `appendSharedContext(rootDir, epicIssue, note)`, `SHARED_CONTEXT_MAX_BYTES = 32768` | New module. Manages `.root/streams/<epic>/shared-context.md`. `getSharedContext` returns full file content as string (empty string if file missing). `appendSharedContext` appends a timestamped note; if the resulting file exceeds `SHARED_CONTEXT_MAX_BYTES`, returns a `{ overflow: true }` signal so the caller can fire a blocker notification rather than silently truncating. File creation creates parent directory if missing. | REQ-004 | A | [ ] |
| 4 | `mcp/mcp-root-board/src/__tests__/sharedContext.test.ts` | create | All cases | Test: get returns empty string when missing, append creates file + parent dir, append round-trips through get, overflow signal fires at threshold. Use tmpdir per test. | REQ-004 | A | [ ] |
| 5 | `mcp/mcp-root-board/src/index.ts` | modify | New tool registrations after `board_status` | Register two new MCP tools: `board_shared_get({ epicIssue })` calls `getSharedContext`, returns the markdown. `board_shared_append({ epicIssue, note })` calls `appendSharedContext`; on overflow fires `sendDiscord('blocker', ...)` and returns `{ overflow: true }` to the caller. | REQ-004 | A | [ ] |
| 6 | `mcp/mcp-root-board/src/index.ts` | modify | `board_start` Project-sync block | Wrap the existing `setProjectStatusInProgress` call (added in #8) in an autonomous-mode-aware path: when called as part of an epic / batch run, also fire `sendDiscord('human_gate', ...)` if the resolved tier triggers a human gate; fire `sendDiscord('blocker', ...)` if the issue's classifier disagreement rule triggers (e.g. tier-1 in a batch run). | REQ-005, REQ-009 | A | [ ] |
...
| 13 | `agents/issue-readiness-grader.md` | create | Full agent template | New specialist agent. Reads issue body + acceptance criteria + labels. Outputs strict JSON: `{ "verdict": "ready" \| "needs-clarification", "confidence": 0..1, "concerns": string[], "questions": string[] }`. Rubric: acceptance criteria present, scope bounded (no "TBD" / "later"), solvability (no unresolved decisions in body), referenced files exist. Returns `needs-clarification` if any rubric item fails; `concerns` lists which, `questions` are concrete questions to ask the human. | REQ-003 | C | [ ] |
| 14 | `skills/root/SKILL.md` | modify | New "Autonomous mode" section after the issue-status-labels section | Add orchestration logic: when `--auto` is supplied, before `board_start` invoke `issue-readiness-grader` agent on the issue (and on every child if `--auto` epic/batch); on any `needs-clarification` verdict, abort and print the questions per issue. Document the orchestration loop for epic/batch: dispatch child subagent → wait for completion → call `board_shared_append` with summary → update PR body checkbox → next child. On any child failure: stop, fire blocker notification, ship partial PR. | REQ-001, REQ-002, REQ-003, REQ-007, REQ-009 | C | [ ] |
...
| Readiness grader is too strict and blocks legitimate work | Medium | Medium | Verdict is JSON with `confidence`; allow `--force` flag to bypass after explicit confirmation. Tune rubric after first 5 real runs. |
...
- [ ] **Manual readiness rejection**: deliberately create an issue with no acceptance criteria, run `/root #N --auto`. Verify: readiness gate fails, run aborts with concrete questions printed, no stream created, no Discord notification (since no work started).
...
1. **Does the readiness grader run on the epic itself, or only on children?** Both probably. An under-specified epic is a hard stop too. Confirm before #14.
2. **What's the exact GraphQL query for sub-issues?** The sub-issues API is recent; the `subIssues` connection on `Issue` is the obvious shape but worth verifying with a one-off `gh api graphql` call against issue #9 before writing #11.
3. **Where does `--force` to bypass readiness grader live?** As a flag on `/root`, or as a `force_after_clarification: true` Boolean in the readiness verdict? Lean: flag, simpler.
4. **Should partial-PR include an explicit "do not merge" marker?** Or trust the user to read the callout? Lean: leave drafts as drafts (`gh pr create --draft`) until all children complete; partial = stays draft until human flips it.
5. **Cross-cutting `nonFatal(label, fn)` helper — refactor in this plan or follow-up?** If it appears in 3+ places after this work, refactor in-plan. If it stays at 2 (Project sync + Discord), defer.
6. **Does the readiness grader read the linked PRD if one exists?** Yes intuitively, but adds complexity. v1: just the issue body; v2 can pull PRD context if it improves verdicts.

_Open questions block implementation — resolve before starting._
