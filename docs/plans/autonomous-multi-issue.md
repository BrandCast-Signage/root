...
| 7 | `mcp/mcp-root-board/src/types.ts` | modify | `Stream` interface | Added `kind: StreamKind`, `epicChildren: number[]`, `epicBranch: string \| null`. Added `StreamKind` type and the `epic-*` lifecycle statuses. Migration backfills all three on existing records (no schema bump — additive). | REQ-001, REQ-002, REQ-007 | B | [x] |
| 8 | `mcp/mcp-root-board/src/board.ts` | modify | `createStream`, new `createEpicStream(parentIssue, kind, children)` | Added `createEpicStream` factory. Status starts at `epic-running`, `autoApprove` defaulted true (autonomous mode), tier forced to tier2 (parent does no code; tier-1 gating happens per child). Branch prefix `feat/epic-` or `chore/batch-` based on `kind`. | REQ-001, REQ-002 | B | [x] |
| 9 | `mcp/mcp-root-board/src/__tests__/board.test.ts` | modify | New describe block `createEpicStream` | 5 cases: kind/children/branch round-trip, batch prefix variant, tier-2 forcing, persistence via readStream, slug fallback when title has no alphanumerics. | REQ-001, REQ-002 | B | [x] |
| 10 | `mcp/mcp-root-board/src/index.ts` | modify | New tool `board_epic_start` | Registered. Resolves children via `getSubIssues` for epic mode, requires explicit list for batch. Rejects empty-children sub-issue lookups with a clear error. Sets root:planning label non-fatally. | REQ-001, REQ-002 | B | [x] |
| 11 | `mcp/mcp-root-board/src/github.ts` | modify | New `getSubIssues(parentIssue): number[]` | Added. GraphQL query against `repository(owner,name).issue(number).subIssues(first:100).nodes{number}`. Owner/repo resolved via `gh repo view --json owner,name`. Throws on failure (non-recoverable for epic mode). | REQ-001 | B | [x] |
| 12 | `mcp/mcp-root-board/src/__tests__/github.test.ts` | modify | New describe block `getSubIssues` | 3 cases: orderly child extraction, empty sub-issues array, propagation of gh errors. | REQ-001 | B | [x] |
| 13 | `agents/issue-readiness-grader.md` | create | Full agent template | Created. 6-item rubric (goal stated, scope bounded, solvability, touchpoints clear, no blocking deps, tier-honest). Strict-JSON verdict shape. Issue-body-only (no PRD lookup). Examples included. Registered in `.claude-plugin/plugin.json` agents list. | REQ-003 | C | [x] |
| 14 | `skills/root/SKILL.md` | modify | Step 0 flag extraction + new "Autonomous Multi-Issue Mode" section | Step 0 now recognizes `--batch` and multi-issue invocations. New section documents: A0 readiness gate (epic + every child), A1 interview loop (3-round cap, `gh issue edit` writes a `## Clarifications` section, no `--force` exists), A2 parent stream creation via `board_epic_start`, A3 per-child dispatch loop (subagent with `isolation: worktree`, commits to epicBranch directly, shared-context updates, PR-body checklist updates), A4 run completion, plus auto-compact resilience semantics. | REQ-001, REQ-002, REQ-003, REQ-007, REQ-009 | C | [x] |
| 15 | ~~`commands/root/root.md`~~ | (n/a) | (no such file) | Argument parsing lives in the skill itself, not in a separate root.md command file. Folded into #14's Step 0 changes. | — | C | [x] |
| 16 | `skills/root/SKILL.md` | modify | `argument-hint` frontmatter | Updated to include the autonomous epic and batch invocation patterns. | REQ-010 | C | [x] |
| 17 | `root.config.example.json` | modify | New `notifications` section | Added `"notifications": { "discord": { "enabled": true, "events": [...], "mention": null } }` with `_comment` pointing at `ROOT_DISCORD_WEBHOOK_URL`. | REQ-005, REQ-008 | C | [x] |
| 18 | `README.md` | modify | New "Autonomous Multi-Issue Mode" section after "Two-Tier Workflow" | Documented epic/batch invocations, readiness gate + interview loop, partial-PR semantics, and the shared-context location. | REQ-010 | C | [x] |
| 19 | `README.md` | modify | New "Notifications" subsection under "Configuration" | Documented `notifications.discord` config + `ROOT_DISCORD_WEBHOOK_URL` env var. Includes the four-event taxonomy and a curl smoke-test snippet. | REQ-008, REQ-010 | C | [x] |
| 20 | `.claude-plugin/plugin.json` | modify | `version` + `agents` | Bumped to `2.4.0`. Added `./agents/issue-readiness-grader.md` to the agents array. | REQ-011 | D | [x] |
| 21 | `.claude-plugin/marketplace.json` | modify | `plugins[0].version` | Bumped to `2.4.0`. | REQ-011 | D | [x] |
| 22 | `gemini-extension.json` | modify | `version` | Bumped to `2.4.0`. (Agents auto-discovered via `agents/*.md` glob — no list change needed.) | REQ-011 | D | [x] |
