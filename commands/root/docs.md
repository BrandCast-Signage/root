...
**This rubric is mandatory. Compliance is not optional. A doc that fails this rubric MUST NOT be written.**

Rubric compliance takes priority over turn count, speed, or efficiency. If a required field (mount path, dependency, side effect) is not in the immediate file, you MUST trace the dependency tree — read imports, grep for route registration, follow the chain — until every required field is confirmed. Never guess, never hedge, never defer a required field for later. An incomplete doc is worse than no doc because it creates false confidence.

**Every doc MUST include:**
- **Purpose**: Why this component exists and what problem it solves — not just "handles X" but why X needs handling
- **Public API surface**: Every exported function, class, type, or endpoint with its signature (name, parameters, return type)
- **Dependencies**: What this component imports/requires from other parts of the system
- **Usage example**: A concrete code snippet or curl command showing how to use this component

**By type, additionally:**
...
**NOT acceptable — these will be rejected:**
- "Handles various operations" or "Provides utility functions" — say which operations, which functions
- "See source code for details" — the doc IS the details
...
**Before writing any doc, self-check:**
1. Did I read the source file AND trace its imports/dependencies?
2. For APIs: do I know the actual mount path (not just the file path)?
3. For mutations: have I identified every side effect (DB, WebSocket, cache, email, billing)?
4. For handlers with fallback logic: have I documented the resolution order?
5. Does every exported function have its signature documented?
6. Is there a concrete usage example?

If any answer is "no" or "I'll verify later" — stop and do the research first. Never write the doc until all answers are "yes."
...
2. Generate frontmatter:
   ...
   ---
   ```
3. Write content that meets the **Doc Quality Rubric** (see top of this file). Read the source code thoroughly — every exported function, every endpoint, every type. The doc must include purpose, full API surface, dependencies, and a usage example. Refer to the rubric's per-type requirements for what else is needed.
...
4. Generate frontmatter:
   ...
   ---
   ```
5. Read source code thoroughly and write content that meets the **Doc Quality Rubric** (see top of this file). Every exported function, endpoint, type, and dependency must be documented. Include a usage example.
6. Write the file
