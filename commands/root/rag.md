...
1. Read `root.config.json` → `ingest.docs`
2. If no config exists, tell the user to run `/root:init` first and stop
3. Run the shared setup, then for each directory in `docs`, ingest the entire directory:
   ```bash
...
2. Run the shared setup, then list indexed files:
   ...
   node "$RAG_BIN" --db-path "$DB_PATH" --cache-dir "$CACHE_DIR" list
   ```
3. Count files on disk in docs directories (use Glob)
4. Show:
   ```
   ...
   Delta:   6 files not yet ingested

   To modify: edit root.config.json → ingest.docs
