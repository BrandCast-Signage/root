...
server.tool("board_start", "Start a new work stream for a GitHub issue. Fetches issue context, creates a stream record, and sets up a git worktree.", {
    ...
    if (groupId !== undefined && groupId.trim().length > 0) {
        ...
        const existing = (0, board_js_1.readStream)(rootDir, issue);
        if (existing !== null) {
            ...
            let worktreePath;
            try {
                worktreePath = (0, worktree_js_1.createWorktree)(rootDir, issue, groupBranch, gid, streamBranch);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                return {
            ...
            const prev = existingGroups[gid] ?? { harness: null, status: "pending", worktreePath: null, branch: null };
            (0, board_js_1.updateStream)(rootDir, issue, {
                groups: {
                    ...existingGroups,
    ...
    (0, board_js_1.createStream)(issueContext, classification.tier, classification.source, classification.reason, rootDir);
    ...
    const updated = (0, board_js_1.updateStream)(rootDir, issue, { worktreePath, branch: branchName });
...
server.tool("board_integrate_groups", "Merge each parallel Execution Group's branch back into the stream branch and prune the group worktrees. Call once after the Tier 1 execution workflow reports `complete`, BEFORE final validation and PR — this is what consolidates the parallel group work onto the single PR branch. Merges run inside the stream worktree (where the stream branch is checked out). A well-formed plan partitions groups by disjoint files, so merges are conflict-free; a conflict means the groups were not actually independent and the run halts for human attention.", {
    ...
    const groupEntries = Object.entries(stream.groups).filter(([, g]) => g.branch !== null && g.branch.length > 0);
    ...
    for (const [gid, g] of groupEntries.sort(([a], [b]) => a.localeCompare(b))) {
        ...
        const result = (0, worktree_js_1.mergeWorktreeInto)(streamWorktree, groupBranch, streamBranch);
        if (!result.success) {
        ...
        if (g.worktreePath !== null) {
            nonFatal(`integrate:removeWorktree:${gid}`, () => (0, worktree_js_1.removeWorktree)(rootDir, g.worktreePath));
        }
        nonFatal(`integrate:deleteBranch:${gid}`, () => (0, worktree_js_1.deleteBranch)(streamWorktree, groupBranch));
        const current = (0, board_js_1.readStream)(rootDir, issue);
        if (current !== null) {
