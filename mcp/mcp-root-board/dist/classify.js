...
exports.scopeSignal = scopeSignal;
...
function scopeSignal(issue) {
    ...
    if (body.length > 0) {
        ...
        const filesHeadingIdx = lines.findIndex((l) => /^##\s+files affected/i.test(l));
        let searchText;
    ...
    const titleAndFirstPara = `${issue.title} ${firstParaLines.join(" ")}`.toLowerCase();
    const hitKeyword = MECHANICAL_KEYWORDS.find((kw) => titleAndFirstPara.includes(kw));
    ...
    if (!mechanical && body.length > 0) {
        const lines = body.split("\n");
        const fixHeadingIdx = lines.findIndex((l) => /^##\s+Fix(?: path)?:?\s*$/i.test(l));
        if (fixHeadingIdx !== -1) {
    ...
    const reasons = [];
    if (small)
        reasons.push(smallReason);
    if (locked)
        reasons.push("locked (AC/done-when)");
    if (mechanical)
...
function classifyTier(issue) {
    ...
    for (const l of lowerLabels) {
        if (TIER1_LABELS.has(l)) {
            const base = { tier: "tier1", reason: `label "${l}" matches Tier 1 policy` };
 ...
 * classification.
 */
function applyDowngrade(issue, base) {
    if (base.tier !== "tier1")
        return base;
